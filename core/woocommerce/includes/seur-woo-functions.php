...
function seur_add_cart_weight( $order_id ) {
	...
	if ( ! empty( $seur_bc2_custom_name ) ) {
		$seur_bc2_custom_name = $seur_bc2_custom_name;
	} else {
		$seur_bc2_custom_name = 'B2C Estándar';                    }
	if ( ! empty( $seur_10e_custom_name ) ) {
		$seur_10e_custom_name = $seur_10e_custom_name;
	} else {
		$seur_10ef_custom_name = 'SEUR 10 Estándar';                }
	if ( ! empty( $seur_10ef_custom_name ) ) {
		$seur_10ef_custom_name = $seur_10ef_custom_name;
	} else {
		$seur_bc2_custom_name = 'SEUR 10 Frío';                    }
	if ( ! empty( $seur_13e_custom_name ) ) {
		$seur_13e_custom_name = $seur_13e_custom_name;
	} else {
		$seur_13e_custom_name = 'SEUR 13:30 Estándar';             }
	if ( ! empty( $seur_13f_custom_name ) ) {
		$seur_13f_custom_name = $seur_13f_custom_name;
	} else {
		$seur_13f_custom_name = 'SEUR 13:30 Frío';                 }
	if ( ! empty( $seur_48h_custom_name ) ) {
		$seur_48h_custom_name = $seur_48h_custom_name;
	} else {
		$seur_48h_custom_name = 'SEUR 48H Estándar';               }
	if ( ! empty( $seur_72h_custom_name ) ) {
		$seur_72h_custom_name = $seur_72h_custom_name;
	} else {
		$seur_72h_custom_name = 'SEUR 72H Estándar';               }
	if ( ! empty( $seur_cit_custom_name ) ) {
		$seur_cit_custom_name = $seur_cit_custom_name;
	} else {
		$seur_cit_custom_name = 'Classic Internacional Terrestre'; }
	if ( ! empty( $seur_2shop_custom_name ) ) {
		$seur_2shop_custom_name = $seur_2shop_custom_name;
	} else {
		$seur_2shop_custom_name = 'SEUR 2SHOP'; }
	if ( ! empty( $seur_courier_int_aereo_paqueteria_custom_name ) ) {
		$seur_courier_int_aereo_paqueteria_custom_name = $seur_courier_int_aereo_paqueteria_custom_name;
	} else {
		$seur_courier_int_aereo_paqueteria_custom_name = 'COURIER INT AEREO PAQUETERIA'; }
	if ( ! empty( $seur_courier_int_aereo_documentos_custom_name ) ) {
		$seur_courier_int_aereo_documentos_custom_name = $seur_courier_int_aereo_documentos_custom_name;
	} else {
		$seur_courier_int_aereo_documentos_custom_name = 'COURIER INT AEREO DOCUMENTOS'; }
	if ( ! empty( $seur_netexpress_int_terrestre_custom_name ) ) {
		$seur_netexpress_int_terrestre_custom_name = $seur_netexpress_int_terrestre_custom_name;
	} else {
		$seur_netexpress_int_terrestre_custom_name = 'NETEXPRESS INT TERRESTRE'; }
...
function seur_filter_price_rate_weight( $package_price, $raterate, $ratepricerate, $countryrate ) {
	...
	if ( $seur_bc2_max_price_field == '0' || ! $seur_bc2_max_price_field ) {
		$seur_bc2_max_price_field = '99999999999';
	}
	if ( $seur_10e_max_price_field == '0' || ! $seur_10e_max_price_field ) {
		$seur_10e_max_price_field = '99999999999';
	}
	if ( $seur_10ef_max_price_field == '0' || ! $seur_10ef_max_price_field ) {
		$seur_10ef_max_price_field = '99999999999';
	}
	if ( $seur_13e_max_price_field == '0' || ! $seur_13e_max_price_field ) {
		$seur_13e_max_price_field = '99999999999';
	}
	if ( $seur_13f_max_price_field == '0' || ! $seur_13f_max_price_field ) {
		$seur_13f_max_price_field = '99999999999';
	}
	if ( $seur_48h_max_price_field == '0' || ! $seur_48h_max_price_field ) {
		$seur_48h_max_price_field = '99999999999';
	}
	if ( $seur_72h_max_price_field == '0' || ! $seur_72h_max_price_field ) {
		$seur_72h_max_price_field = '99999999999';
	}
	if ( $seur_cit_max_price_field == '0' || ! $seur_cit_max_price_field ) {
		$seur_cit_max_price_field = '99999999999';
	}
	if ( $seur_2SHOP_max_price_field == '0' || ! $seur_2SHOP_max_price_field ) {
		$seur_2SHOP_max_price_field = '99999999999';
	}

	if ( $seur_courier_int_aereo_paqueteria_max_price_field == '0' || ! $seur_courier_int_aereo_paqueteria_max_price_field ) {
		$seur_courier_int_aereo_paqueteria_max_price_field = '99999999999';
	}
	if ( $seur_courier_int_aereo_documentos_max_price_field == '0' || ! $seur_courier_int_aereo_documentos_max_price_field ) {
		$seur_courier_int_aereo_documentos_max_price_field = '99999999999';
	}
	if ( $seur_netexpress_int_terrestre_max_price_field == '0' || ! $seur_netexpress_int_terrestre_max_price_field ) {
		$seur_netexpress_int_terrestre_max_price_field = '99999999999';
	}
...
function seur_post_formats_filter_to_woo_order_administration() {
	...
	if ( 'shop_order' === $post_type ) {
		...
		if ( ! empty( $seur_10e_custom_name ) ) {
			...
			$seur_10e_custom_name = 'SEUR 10 Estándar';                }
		if ( ! empty( $seur_10ef_custom_name ) ) {
			$seur_10ef_custom_name = $seur_10ef_custom_name;
		} else {
			$seur_10ef_custom_name = 'SEUR 10 Frío';                    }
		if ( ! empty( $seur_13e_custom_name ) ) {
			$seur_13e_custom_name = $seur_13e_custom_name;
		} else {
			$seur_13e_custom_name = 'SEUR 13:30 Estándar';             }
...
<!--            <option value="all_seur"><?php esc_html_e( 'All Seur Shippments', 'seur' ); ?></option> -->
		<?php
...
function seur_filter_orders_by_shipping_method_query( $vars ) {
	...
	if ( 'shop_order' === $typenow && isset( $_GET['_shop_order_seur_shipping_method'] ) ) {
		...
		foreach ( $seur_shipments as $shippment ) {
			if ( sanitize_title( $shippment ) === $_GET['_shop_order_seur_shipping_method'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$vars['meta_key']   = '_seur_shipping_method_service';
				$vars['meta_value'] = sanitize_title( $shippment );
			} elseif ( 'seur' === $_GET['_shop_order_seur_shipping_method'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
